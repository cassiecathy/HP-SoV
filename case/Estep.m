function [X_smooth, Exx, Exy, llh, X_filter,xp,Vp] = Estep(model, Y)
% Input:
...
for i = 1:nsam
    % filtering forward
    PC = V1*B0{1}';
    S = B0{1}*PC+sigma1*eye(ntime(1)*nsensor(1));
    K = PC/S;
    ...
    for s = 2:T    
        [X_filter{s}(:,i), Vxx_filter{s}(:,:,i), xp{s}(:,i), Vp{s}(:,:,i), llh(s,i)] = ...
            forwardUpdate(Y{s}(:,i), X_filter{s-1}(:,i), Vxx_filter{s-1}(:,:,i), F{s-1}, sigma2*eye(npc(s)*nsensor(s)), B0{s}, sigma1*eye(ntime(s)*nsensor(s)));
    end
